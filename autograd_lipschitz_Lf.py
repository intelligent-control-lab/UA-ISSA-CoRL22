...
def main():
    ...
    grad_dynamics_px = grad(dynamics_px)
    grad_dynamics_py = grad(dynamics_py)
    grad_dynamics_theta = grad(dynamics_theta)
    grad_dynamics_v = grad(dynamics_v)
    grad_dynamics_w = grad(dynamics_w)

    Lipcshitz_f = 0
    for i in tqdm(range(len(X_tau))):
        for j in range(len(U_tau)):
            input_for_dynamics = np.concatenate([X_tau[i], U_tau[j]])
            grad_px = grad_dynamics_px(input_for_dynamics)
            grad_py = grad_dynamics_py(input_for_dynamics)
            grad_theta = grad_dynamics_theta(input_for_dynamics)
            grad_v = grad_dynamics_v(input_for_dynamics)
            grad_w = grad_dynamics_w(input_for_dynamics)
            grad_list = np.array([grad_px, grad_py, grad_theta, grad_v, grad_w])
            L = np.sum(np.abs(grad_list))
            if L > Lipcshitz_f:
                Lipcshitz_f = L
                print(input_for_dynamics)
                print(grad_list)
                print(Lipcshitz_f)
            if i % 100 == 0:
                print(Lipcshitz_f)
